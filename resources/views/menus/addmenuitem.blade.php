...
@section('content')
<h3>Add New Menu Item(s) to {{ $menu->name }}</h3>

{!! Form::open(['route' => ['menus.update', 'id' => $menu->id], 'method' => 'PATCH', 'class' => 'col s12']) !!}
    ...
    <table id="mpv_table" class="responsive-table">
      <tbody id="more_mpv">
        <tr id='mpv_row1'>
          ...
          <td>
            <label>
              <input id='mpv_service_unclaimed' name='mpv_service[1][unclaimed]' type='checkbox' />
              <span>Add to Unclaimed Services</span>
            </label>
          </td>
